...
def train(datapath, resultspath, modelpath, fstub, loss, lr=1e-3, batchsize=10, batchlength=256, normalize=False, quantize=True, sparse=False, num_embeddings=1024, embedding_dim=128):
    print("DEVICE:", device)
    ### Declare model ###
...
if __name__ == "__main__":
    ...
    print("File stub:%s",fstub)
    print("Sparse:%s\nNormalize:%s\nQuantize:%s\n", str(sparse), str(normalize), str(quantize))
    print("Batch size:%s\nBatch length:%s\n", batchsize, batchlength)
    print("Embedding dimension:%s\nNumber of embeddings:%s\n", embeddim, numembed)
    print("Learning Rate:%s", lr)
    
    train(datadir, outdir, modeldir, fstub, loss, lr=lr, batchsize=batchsize, batchlength=batchlength, normalize=normalize, quantize=quantize, sparse=sparse, num_embeddings=numembed, embedding_dim=embeddim)
